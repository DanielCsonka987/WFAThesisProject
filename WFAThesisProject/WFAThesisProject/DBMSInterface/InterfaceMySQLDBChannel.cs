...
namespace WFAThesisProject
{
    class InterfaceMySQLDBChannel
    {
        ...
        #region prepared query methods 9 - oneField, dataTable, DMQuery, ScalarQuery, List<string[]>, string[]
        ...
        /// <param name="paramNum"/>the number of paramteres</param>
        /// <returns></returns>
        public string[] execPrepQueryOneRowInStringArr(string query, List<KeyValuePair<string, string>> param, int paramNum)
        {
            string[] rowValue = null;
            try
            {
                ...
                if (connection.State != ConnectionState.Open)
                {
                    setErrorDataAndShow("Adatbázis kapcsolat nincs megnyitva (ExecPrepSA). Lekérdezést nem lehet végrehajtani.\n");
                    return rowValue;
                }
                if (!isExecutableQuery(query))
                {
                ...
                if (!isPreparedParamCompletness(param, paramNum))
                {
                    setErrorDataAndShow("Lekérdezési információk sérültek (ExecPrepSA). Lekérdezést nem lehet végrehajtani.\n");
                    return rowValue;
                }
                MySqlCommand cmd;
                try
                ...
                {
                    setErrorDataAndShow("Adatbázis lekérdezés hiba (ExecPrepSA).\n" + me.Message/*+me.Number*/);
                }
                finally
                {
                }
