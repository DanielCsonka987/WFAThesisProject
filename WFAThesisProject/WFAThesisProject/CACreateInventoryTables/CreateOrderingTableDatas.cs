    ...
    class CreateOrderingTableDatas
    {
        ...
        public static List<string> createordering(List<string[]> tempStore, List<string[]> tempRequests)
        {
            ...
            for (int i = 0; i < tempStore.Count; i++)
            {
                ...
                value += Convert.ToInt32(tempStore[i][1]);  //adding the amount of store
                string row = "";
                row += "(" + tempStore[i][0] + ", ";    //the product DB stripping ID -> refers to 'termek_min_id' and 'termek_kiszerel'
                row += "3, ";                   //who ordered
