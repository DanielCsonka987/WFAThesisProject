    ...
    public class ProductsModelReadIn
    {
        ...
        public string getWhoModifiedRecord(int indexOfUser)
        {
            ...
            try
            {
                ...
                mdi.openConnection();
                result = mdi.execPrepQueryOneRowInStringArr(queryWhoChangedTheRecord, parameters, 1);
                mdi.closeConnection();
            }
            catch (Exception e)
            {
                mdi.closeConnection();
                throw new ErrorServiceWhoDeleted("A rekord törlője kereséséhez kérés megalkotása sikertelen (ModProdListQual)");
            }
            if (result == null)
            {
                throw new ErrorServiceWhoDeleted("A rekord módosítójának visszakeresése sikertelen (ModProdListQual)");
            }
